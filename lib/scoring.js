...
function scorePosts(tweets) {
  ...
  // The scan caps at 100 posts, so very active accounts may only have
// 12-20 days of real data fetched, not the full 30. Dividing by a
// hardcoded 30 punishes high-volume posters for days we never even
// retrieved. Instead, measure against the window we actually have.
const postDayNumbers = tweets.map((t) => dayNumber(t.createdAt));
const oldestDay = Math.min(...postDayNumbers);
const newestDay = Math.max(...postDayNumbers);
const windowDays = clamp(newestDay - oldestDay + 1, 1, 30);

const distinctArcDays = arcPostDayNumbers.size;
const presenceScore = clamp((distinctArcDays / windowDays) * 100, 0, 100);

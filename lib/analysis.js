...
function isRetweet(text) {
  return /^rt @/i.test((text || "").trim());
}

// "link share" posts (sharing someone else's article/thread) aren't
// really the person's own authored voice, so they're tracked separately
// and never used to drive "best format" or "best time" recommendations.
// Doing so was skewing both, someone who mostly links out at night but
// writes their real thoughts in the morning was getting told "you're a
// night poster," which wasn't true of their own writing at all.
const AUTHORED_TYPES = new Set(["question", "hot take", "announcement", "short post", "detailed post"]);
...
function classifyPost(text) {
  ...
  if (text.length > 180) return "detailed post";
  return "short post";
}
...
function analyzeContent(tweets) {
  const original = (tweets || []).filter((t) => !isRetweet(t.text));
  ...
  if (!original || original.length === 0) {
    return {
      ...
      needsImprovement: [],
      suggestions: [
        "Every post in this window was a retweet, so there's no original content yet to break down."
      ]
    };
  }
  ...
  for (const post of sorted) {
    ...
    if (type === "link share") {
      const { count: arcMentionCount } = countArcMentions(post.text);
      if (arcMentionCount > 0) linkShareCount += 1;
      continue;
    }
  ...
  const typeStats = Object.entries(byType)
    .map(([type, v]) => ({
      type,
      count: v.count,
      share: pct(v.count, original.length),
  ...
  const topPost = authoredPosts.length
    ? authoredPosts.reduce(
        (best, p) => (weightedEngagement(p) > weightedEngagement(best) ? p : best),
        authoredPosts[0]
      )
    : null;

  const linkShareShare = pct(linkShareCount, original.length);
...
function buildBullets(stats) {
  ...
  if (bestType && bestType.count >= 2) {
    workingWell.push(
      `Your "${bestType.type}" posts are your strongest format, averaging ${bestType.avgEngagement} engagement across ${bestType.count} posts.`
    );
    suggestions.push(
      `Write more "${bestType.type}" posts, that format is already outperforming everything else you write.`
    );
  }

  if (bestWindow) {
    workingWell.push(`Your own posts get the most engagement in the ${bestWindow.window}.`);
    suggestions.push(`Save your best writing for the ${bestWindow.window}, that is your strongest window.`);
  }
  ...
  if (linkShareShare >= 15) {
    workingWell.push(`You share a good amount of Arc-related content and links (${linkShareShare}% of your posts).`);
  }
  ...
  if (worstType && worstType.type !== bestType?.type && worstType.count >= 2) {
    needsImprovement.push(
      `Your "${worstType.type}" posts are your weakest format, averaging only ${worstType.avgEngagement} engagement.`
    );

...
async function mintBadgeTo(recipientAddress, tokenId) {
  ...
  // The contract rejects an empty URI outright ("execution reverted: empty
  // uri"), confirmed directly from Circle's transaction error details.
  // This is a placeholder pointing at the app itself for now, real
  // per-badge metadata (name, image, tier) hosted at a proper JSON
  // endpoint is a good next improvement, but not required to mint.
  const placeholderUri = `https://arc-conviction-app.vercel.app/badge/${tokenId}`;
  ...
  const response = await walletsClient.createContractExecutionTransaction({
    ...
    abiParameters: [recipientAddress, placeholderUri],
